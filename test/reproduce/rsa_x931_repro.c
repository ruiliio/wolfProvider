/* rsa_x931_repro.c - Test case to reproduce RSA X931 signature verification issue */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int test_rsa_x931_sign_verify(OSSL_LIB_CTX *libCtx, const char *provName, 
                                    EVP_PKEY *pkey, const char *hashName, 
                                    int iteration) {
    int ret = 0;
    ...
    if (EVP_DigestSignInit_ex(mdCtx, &pkeyCtx, hashName, libCtx, provName, pkey, NULL) != 1) {
        fprintf(stderr, "Failed to initialize signing with %s\n", provName);
    ...
    if (EVP_DigestSign(mdCtx, sig, &sigLen, data, sizeof(data)) != 1) {
        fprintf(stderr, "Failed to sign data with %s (iteration %d, hash %s)\n", 
                provName, iteration, hashName);
        check_openssl_errors();
        goto cleanup;
    }

    /* Reset the context for verification with OpenSSL */
    EVP_MD_CTX_reset(mdCtx);
    
    /* Initialize for verification with OpenSSL default provider */
    if (EVP_DigestVerifyInit_ex(mdCtx, &pkeyCtx, hashName, NULL, "default", pkey, NULL) != 1) {
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < iterations; i++) {
        for (h = 0; h < HASH_TYPES; h++) {
            if (!test_rsa_x931_sign_verify(libCtx, "libwolfprov", pkey, hashNames[h], i)) {
                failures++;
