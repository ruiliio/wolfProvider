/* rsa_x931_asan_test2.c - Test case for RSA X931 signature verification with ASAN */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int test_rsa_x931_sign_verify(const EVP_MD *md, const unsigned char *data, 
                                    size_t data_len)
{
    EVP_PKEY *pkey = NULL;
    BIO *bio = NULL;
    ...
    if (EVP_DigestSignInit(sign_ctx, &pkey_ctx, md, NULL, pkey) != 1) {
    ...
    if (EVP_DigestVerifyInit(verify_ctx, &pkey_ctx, md, NULL, pkey) != 1) {
    ...
    ret = EVP_DigestVerifyFinal(verify_ctx, signature, sig_len);
    if (ret != 1) {
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < iterations; i++) {
        ...
        /* Test with SHA-1 */
        if (test_rsa_x931_sign_verify(EVP_sha1(), data, sizeof(data))) {
            success_count++;
        } else {
        ...
        /* Test with SHA-256 */
        if (test_rsa_x931_sign_verify(EVP_sha256(), data, sizeof(data))) {
            success_count++;
        } else {
        ...
        /* Test with SHA-384 */
        if (test_rsa_x931_sign_verify(EVP_sha384(), data, sizeof(data))) {
            success_count++;
        } else {
        ...
        /* Test with SHA-512 */
        if (test_rsa_x931_sign_verify(EVP_sha512(), data, sizeof(data))) {
            success_count++;
        } else {
